...
/* Code goes below */
  const stringJSON = fetchPokemonData();
  const jsonObj = JSON.parse(stringJSON);
  
  console.log(jsonObj);
  
  const sprites = jsonObj.sprites;
  
  const article = document.querySelector("article");
  let pokemon = document.createElement("sprites");
  
  for (const [key, value] of Object.entries(sprites)) {
      let listItem = document.createElement("H3");
      listItem.innerHTML = `<img src=${value}> `;
      pokemon.append(listItem);
  }
  
  article.append(pokemon);
